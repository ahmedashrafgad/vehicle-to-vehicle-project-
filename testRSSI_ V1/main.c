...
#define d1 beaconDevice[0].distance
#define d2 beaconDevice[1].distance
#define d3 beaconDevice[2].distance
#define d4 beaconDevice[3].distance
...
void location(struct beacon beaconDevice[3]);

int main(void)
{
		usart0_init(9600);
		usart0_puts("Start of application\n") ;
		CarControlInit();
		
		unsigned int xValue  = 0, yValue = 0 ;
		sei();
	
	beaconDevice[0].address = 12017048; //melos lab
	beaconDevice[1].address = 1937820;  //melos mob
	beaconDevice[2].address = 1156291   ; //sara mob
	beaconDevice[3].address = 316796    ; //shakeer lab
	deviceInfo[0].address = 16142873 ;
	...
	while (1)
	{
		...
		for(j = 0; j<4 ; j++)
		{
			for(i = 0; i<9 ; i++)
			{
				if(deviceSData[i].address == beaconDevice[j].address)
				{
					beaconDevice[j].rssi = deviceSData[i].rssi ;
		...
		for(j=0 ; j<4 ;j++)
		{
			beaconDevice[j].distance = pow(10.0, (76.5-beaconDevice[j].rssi)/20.0);

		}

		location(beaconDevice);
...
void location(struct beacon beaconDevice[3])
{
	...
	for (c = 0; c < 4; c++)
	printf("%0.3f\t", beaconDevice[c].distance);
